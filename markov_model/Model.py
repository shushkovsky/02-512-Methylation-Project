...
def construct_state_matrix(df, methyl_names, age, methyl, locations):
    ...
    for buckets in range(age):
        age_buckets += [
            (
                df.age.quantile(1 / age * buckets),
                df.age.quantile(1 / age * (buckets + 1)),
            )
        ]
        ...
        for (lb, ub) in age_buckets:
            if row["age"] <= ub and row["age"] >= lb:
                for methylsites in range(methyl):
                    states_matrix_temp[states_matrix_index][methylsites] += [
                        row[methyl_names[methylsites]]
                    ]
...
def construct_state_matrix_prune(df, methyl_names, age, methyl, locations):
    ...
    for methyl_buckets_index in range(methyl):
        ...
        # for location_index in range(locations):
            # methyl_buckets_mini += [
                # (
                    # getattr(df, methyl_name_at_index).quantile(
                        # 1 / locations * location_index
                    # ),
                    # getattr(df, methyl_name_at_index).quantile(
                        # 1 / locations * (location_index + 1)
                    # ),
                # )
            # ]
    ...
    # methyl_buckets = [
        # (2/20 * i - 1, 2/20 * (i+1) - 1) for i in range(20)
    # ]
    #print(len(methyl_buckets[0]))
    print(methyl_buckets[0])
            ...
            if row["age"] <= ub and row["age"] >= lb:
                for methylsites in range(methyl):
                    ...
                    for (mlb, mub) in methyl_buckets[methylsites]:
                        if (
                            row[methyl_names[methylsites]] <= mub
                            and row[methyl_names[methylsites]] >= mlb
                        ):
                            states_matrix_temp[states_matrix_age_index][methylsites][
                                states_matrix_methyl_index
                            ] += [row[methyl_names[methylsites]]]
    ...
    for age_group in range(age):
        for methyl_site in range(methyl):
            ...
            for methyl_positions in range(locations):
                if (
                    len(states_matrix_temp[age_group][methyl_site][methyl_positions])
                    != 0
                ):
                    average_val = sum(
                        states_matrix_temp[age_group][methyl_site][methyl_positions]
                    ) / len(
                        states_matrix_temp[age_group][methyl_site][methyl_positions]
                    )
                else:
            ...
            for positions_2 in range(locations):
                ...
                for positions_2_newage in range(locations):
                    ...
                    for positions_2_newage_2 in range(locations):
                        if (
                            state_matrix[ages_2 + 1][methylsites_2][
                                positions_2_newage_2
                            ]
                            != 0
                        ):
                            sum_of_transition += (
                                state_matrix[ages_2][methylsites_2][positions_2_newage]
                                / state_matrix[ages_2 + 1][methylsites_2][
                                    positions_2_newage_2
                                ]
                            )
                    ...
                    if (
                        sum_of_transition != 0
                        and state_matrix[ages_2 + 1][methylsites_2][positions_2_newage]
                        != 0
                    ):
                        value_at_index += [
                            1
    ...
    for age_group in transition_matrix:
        ...
        for outgoing_locations in age_group:
            ...
            for index in range(len(transposed_list)):
                ...
                """
                transition_matrix_mod_outgoing_list += [
                    sum(transposed_list[index]) / len(transposed_list[index])
                ]
...
def list_of_predicted_actual_age(
    df, state_matrix, age_groups, methylsites, age_buckets
):
        ...
        for age_index in range(len(state_matrix_transposed)):
            for location_index in range(len(state_matrix_transposed[age_index])):
                ...
                for methyl_index in range(len(methylsites)):
                    ...
                    distance += abs(
                        row[methylsites[methyl_index]]
                        - state_matrix_transposed[age_index][location_index][
                            methyl_index
                        ]
                    )
...
def condition_prediction(
    state_matrix_healthy,
    transition_matrix_healthy,
    state_matrix_disease,
    transition_matrix_disease,
    age_buckets_healthy,
    methyl_buckets_healthy,
    age_buckets_diseased,
    methyl_buckets_diseased,
    ...
    error_of_healthy = error_of_prediction_actual(
        past_age,
        current_age,
        methyl_buckets_healthy,
        age_buckets_healthy,
    ...
    error_of_disease = error_of_prediction_actual(
        past_age,
        current_age,
        methyl_buckets_diseased,
        age_buckets_diseased,
...
def error_of_prediction_actual(
    past_age,
    current_age,
    methyl_buckets,
    age_buckets,
    ...
    for methyl_index in range(len(methyl_buckets)):
        for location_index in range(len(methyl_buckets[methyl_index])):
            ...
            if (
                past_methylation_data[methyl_index] > methyl_lb
                and past_methylation_data[methyl_index] < methyl_ub
            ):
    ...
    if age_buckets[-1][1] < current_age or age_buckets[0][0] > current_age:
        return float("inf")
    while (
        current_age < age_buckets[loop_age_index][0]
        or current_age > age_buckets[loop_age_index][1]
    ):
        next_location_index = transition_matrix[loop_age_index][
            loop_location_index
        ].index(max(transition_matrix[loop_age_index][loop_location_index]))
    ...
    for methyl_site in range(len(current_methylation_data)):
        score += (
            current_methylation_data[methyl_site]
            - state_matrix[loop_age_index][methyl_site][loop_location_index]
        )

    return score


def predictions_of_conditions(
    longitudinal_testing_df,
    state_matrix_healthy,
    transition_matrix_healthy,
    state_matrix_diseased,
    transition_matrix_diseased,
    age_buckets_healthy,
    methyl_buckets_healthy,
    age_buckets_diseased,
    methyl_buckets_diseased,
...
def graph_conditions(predicted_condition, actual_condition):
    ...
    plt.xticks(
        [r + barWidth for r in range(len(bar_plot_actual))],
    ...
    print(
        "Healthy:",
        "Predicted",
        predicted_condition.count(1),
        "Actual",
        actual_condition.count(1),
    )
    print(
        "Diabetes:",
        "Predicted",
...
def main():
    ...
    df_disease_all = pd.read_csv(
        "Input_Datasets/Diabetes_Set.csv"
    )
    df_healthy_all = pd.read_csv(
        "Input_Datasets/Healthy_Set.csv"
    )
    list_of_methylation_sites_all = list(df_healthy_all.columns)
    list_of_methylation_sites_all = list_of_methylation_sites_all[
        1 : len(list_of_methylation_sites_all)
    ]
    for col in list_of_methylation_sites_all:
        df_disease = df_disease_all[['age', col]]
        df_healthy = df_healthy_all[['age', col]]
        list_of_methylation_sites = [col]
        number_of_methylation_sites = len(list_of_methylation_sites)
        number_of_age_groups = 6 # int(min(len(df_disease), len(df_healthy)) * 0.5)
        number_of_locations_at_each_age_group = NUM_METHYL_BUCKETS
        # int(
            # (number_of_age_groups * number_of_methylation_sites) / 2
        # )
        ...
        print(
            "Number of Location Positions at each Age Group",
            number_of_locations_at_each_age_group,
        )
        ...
        # make markov model
        (
            state_matrix_healthy_pre_prune,
            age_buckets_healthy_pre_prune,
        ) = construct_state_matrix(
            df_healthy,
            list_of_methylation_sites,
            number_of_age_groups,
            number_of_methylation_sites,
            number_of_locations_at_each_age_group,
        )
        transition_matrix_healthy_pre_prune = construct_transition_matrix(
            state_matrix_healthy_pre_prune,
            number_of_age_groups,
            number_of_methylation_sites,
            number_of_locations_at_each_age_group,
        ...
        print(
            ...
            len(state_matrix_healthy_pre_prune),
            len(state_matrix_healthy_pre_prune[0]),
            len(state_matrix_healthy_pre_prune[0][0]),
        )
        print(
            "Transition Matrix Dimensions",
            len(transition_matrix_healthy_pre_prune),
            len(transition_matrix_healthy_pre_prune[0]),
            len(transition_matrix_healthy_pre_prune[0][0]),
        )
        # save model to .csv file
        df_state_healthy_pre_prune = pd.DataFrame(state_matrix_healthy_pre_prune)
        df_state_healthy_pre_prune.to_csv(
            f"Markov_Models/df_state_healthy_pre_prune_{col}.csv"
        )
        df_transition_healthy_pre_prune = pd.DataFrame(transition_matrix_healthy_pre_prune)
        df_transition_healthy_pre_prune.to_csv(
            f"Markov_Models/df_transition_healthy_pre_prune_{col}.csv"
        )
        # predict age
        (
            predicted_ages_healthy_pre_prune,
            actual_ages_healthy_pre_prune,
        ) = list_of_predicted_actual_age(
            df_healthy,
            state_matrix_healthy_pre_prune,
            number_of_age_groups,
            list_of_methylation_sites,
        ...
        print(
            "Healthy_Pre_Prune_Error",
            average_distance_apart(
                predicted_ages_healthy_pre_prune, actual_ages_healthy_pre_prune
            ),
        )
        ...
        # make markov model
        (
            state_matrix_diseased_pre_prune,
            age_buckets_diseased_pre_prune,
        ) = construct_state_matrix(
            df_disease,
            list_of_methylation_sites,
            number_of_age_groups,
        ...
        transition_matrix_diseased_pre_prune = construct_transition_matrix(
            state_matrix_diseased_pre_prune,
            number_of_age_groups,
            number_of_methylation_sites,
            number_of_locations_at_each_age_group,
        ...
        print(
            "State Matrix Dimensions",
            len(state_matrix_diseased_pre_prune),
            len(state_matrix_diseased_pre_prune[0]),
            len(state_matrix_diseased_pre_prune[0][0]),
        )
        print(
            "Transition Matrix Dimensions",
            len(transition_matrix_diseased_pre_prune),
            len(transition_matrix_diseased_pre_prune[0]),
            len(transition_matrix_diseased_pre_prune[0][0]),
        )
        # save model to .csv file
        df_state_disease_pre_prune = pd.DataFrame(state_matrix_diseased_pre_prune)
        df_state_disease_pre_prune.to_csv(
            f"Markov_Models/df_state_disease_pre_prune_{col}.csv"
        )
        df_transition_disease_pre_prune = pd.DataFrame(transition_matrix_diseased_pre_prune)
        df_transition_disease_pre_prune.to_csv(
            f"Markov_Models/df_transition_disease_pre_prune_{col}.csv"
        )
        # predict age
        (
            predicted_ages_disease_pre_prune,
            actual_ages_disease_pre_prune,
        ) = list_of_predicted_actual_age(
            df_disease,
            state_matrix_diseased_pre_prune,
            number_of_age_groups,
        ...
        print(
            "Diseased_Pre_Prune_Error",
            average_distance_apart(
                predicted_ages_disease_pre_prune, actual_ages_disease_pre_prune
            ),
        )
        ...
        transition_matrix_healthy = construct_transition_matrix(
            state_matrix_healthy,
            number_of_age_groups,
            number_of_methylation_sites,
            number_of_locations_at_each_age_group,
        ...
        print(
            "State Matrix Dimensions",
            len(state_matrix_healthy),
            len(state_matrix_healthy[0]),
            len(state_matrix_healthy[0][0]),
        )
        print(
            "Transition Matrix Dimensions",
            len(transition_matrix_healthy),
            len(transition_matrix_healthy[0]),
            len(transition_matrix_healthy[0][0]),
        )
        # save model to .csv file
        df_state_healthy_post_prune = pd.DataFrame(state_matrix_healthy)
        df_state_healthy_post_prune.to_csv(
            f"Markov_Models/df_state_healthy_post_prune_{col}.csv"
        )
        df_transition_healthy_post_prune = pd.DataFrame(transition_matrix_healthy)
        df_transition_healthy_post_prune.to_csv(
            f"Markov_Models/df_transition_healthy_post_prune_{col}.csv"
        )
        # predict age
        predicted_ages_healthy, actual_ages_healthy = list_of_predicted_actual_age(
            df_healthy,
            state_matrix_healthy,
            number_of_age_groups,
            list_of_methylation_sites,
        ...
        print(
            "Healthy_Post_Prune_Error",
            average_distance_apart(predicted_ages_healthy, actual_ages_healthy),
        )
        ...
        transition_matrix_diseased = construct_transition_matrix(
            state_matrix_diseased,
            number_of_age_groups,
            number_of_methylation_sites,
            number_of_locations_at_each_age_group,
        ...
        print(
            "State Matrix Dimensions",
            len(state_matrix_diseased),
            len(state_matrix_diseased[0]),
            len(state_matrix_diseased[0][0]),
        )
        print(
            "Transition Matrix Dimensions",
            len(transition_matrix_diseased),
            len(transition_matrix_diseased[0]),
            len(transition_matrix_diseased[0][0]),
        )
        # save model to .csv file
        df_state_disease_post_prune = pd.DataFrame(state_matrix_diseased)
        df_state_disease_post_prune.to_csv(
            f"Markov_Models/df_state_disease_post_prune_{col}.csv"
        )
        df_transition_disease_post_prune = pd.DataFrame(transition_matrix_diseased)
        df_transition_disease_post_prune.to_csv(
            f"Markov_Models/df_transition_disease_post_prune_{col}.csv"
        )
        # predict age
        predicted_ages_disease, actual_ages_disease = list_of_predicted_actual_age(
            df_disease,
            state_matrix_diseased,
            number_of_age_groups,
            list_of_methylation_sites,
        ...
        print(
            "Healthy_Pre_Prune_Error",
            average_distance_apart(predicted_ages_disease, actual_ages_disease),
        )
        # graph_ages(predicted_ages_disease, actual_ages_disease, True)

        # # Condition Prediction
        # print("\n Condition Prediction")
        # df_longitudinal_methylation = pd.read_csv(
            # "Input_Datasets/Longitude_Set.csv"
        # )
        # predicted_condition, actual_condition = predictions_of_conditions(
            # df_longitudinal_methylation,
            # state_matrix_healthy,
            # transition_matrix_healthy,
            # state_matrix_diseased,
            # transition_matrix_diseased,
